...
export class WeeklyScheduleComponent {
  ...
  /** Whether there's anything worth showing in the પ્રવચન વિષય section for this week. */
  ...
  loadTopic(): void {
    this.weeklyTopicService.getForWeek(toIsoDate(this.weekStart())).subscribe({
      ...
      // Non-critical section: silently leave it blank if it fails to load rather than
      // blocking the rest of the weekly schedule.
